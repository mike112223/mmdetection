...
from lib import CocoDataset, BboxResize, AnchorGenerator, bbox_overlaps, MaxIoUAssigner
...
def assigner_iou_analysis():
    ...
    for i, img in enumerate(data):
        ...
        nums = np.concatenate([nums,
                               [0] * ((max(np.concatenate([area_index['large'], area_index['medium'], area_index['small']])) + 1) - len(nums))])
        ...
        nums_small_list.append(nums[area_index['small']])
    ...
    num_small = np.concatenate(nums_small_list, axis=0)
    # num_8small = np.concatenate(nums_8small_list, axis=0)
    # num_10small = np.concatenate(nums_10small_list, axis=0)
    # num_12small = np.concatenate(nums_12small_list, axis=0)
    # num_14small = np.concatenate(nums_14small_list, axis=0)
    # num_16small = np.concatenate(nums_16small_list, axis=0)
    # num_24small = np.concatenate(nums_24small_list, axis=0)
    # num_32small = np.concatenate(nums_32small_list, axis=0)
    ...
    for i, limit in enumerate(['all', 'large', 'medium', 'small']):
