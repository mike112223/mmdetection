...
class AnaRetinaHead(AnchorHead):
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        iou = label_weights.new_zeros(labels.shape)
        ...
        # x1, y1, x2 ,y2
        decode_bbox_pred = self.bbox_coder.decode(anchors,
                                                  bbox_pred)

        gts = self.bbox_coder.decode(anchors,
                                     bbox_targets)

        iou[pos_inds] = bbox_overlaps(
            decode_bbox_pred.detach(),
            gts,
            is_aligned=True)
