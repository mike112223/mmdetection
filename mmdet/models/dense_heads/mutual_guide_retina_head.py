...
from mmdet.core import (anchor_inside_flags, images_to_levels,
                        multi_apply, unmap, force_fp32, bbox_overlaps)
...
class MutualGuideRetinaHead(AnchorHead):
    ...
    def __init__(self,
        ...
        super(MutualGuideRetinaHead, self).__init__(
            num_classes,
            in_channels,
    ...
    def _get_targets_single(self,
        ...
        anchor_assign_result, prop_assign_result = self.assigner.assign(
            anchors, proposals, gt_bboxes, gt_bboxes_ignore,
            None if self.sampling else gt_labels)
        sampling_result = self.sampler.sample(anchor_assign_result, anchors,
                                              gt_bboxes)
        prop_sampling_result = self.sampler.sample(prop_assign_result, proposals,
                                                   gt_bboxes)
        ...
        prop_pos_inds = prop_sampling_result.pos_inds
        prop_neg_inds = prop_sampling_result.neg_inds
        ...
        if len(prop_pos_inds) > 0:
            if gt_labels is None:
                # only rpn gives gt_labels as None, this time FG is 1
                labels[prop_pos_inds] = 1
            else:
                labels[prop_pos_inds] = gt_labels[
                    prop_sampling_result.pos_assigned_gt_inds]
            if self.train_cfg.pos_weight <= 0:
                label_weights[prop_pos_inds] = 1.0
            else:
                label_weights[prop_pos_inds] = self.train_cfg.pos_weight
        if len(prop_neg_inds) > 0:
            label_weights[prop_neg_inds] = 1.0
