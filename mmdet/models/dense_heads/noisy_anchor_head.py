...
class NoisyAnchorHead(AnchorHead):
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        labels_c = label_weights.new_zeros(labels.shape)
        ...
        if len(pos_inds) > 0:
            ...
            loc_a = bbox_overlaps(
                ...
                pos_bboxes,
                is_aligned=True)
            cls_c = cls_score.sigmoid()[labels[pos_inds]].reshape(-1).detach()

            labels_c[pos_inds] = self._weight(loc_a, cls_c)

            re_weights = torch.pow(self._weight(self._nonlinear(loc_a),
                                                self._nonlinear(cls_c)), 2)

            label_weights[pos_inds] = re_weights
            bbox_weights[pos_inds] = re_weights.reshape(-1, 1)

        loss_cls = self.loss_cls(
            cls_score, (labels, labels_c),
